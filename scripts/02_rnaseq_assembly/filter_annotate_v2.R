...
## Load reference GTF
gtf_reference <- rtracklayer::import.gff(gtf_ref_loc, colnames = c(
  "type",
  "source",
  "gene_id",
  "gene_name",
...
## Remove x and i transcripts with same-strand reference overlap (x should be opposite strand, same-strand i is not interesting)
## Also sets gene_id
...
### Filter for x and i transcripts that have same strand as reference
gtf_novel <- gtf_novel_joined %>%
  group_by(transcript_id) %>%
  ...
  mutate(
    # Determine the gene_id based on ref_gene_id_annotated and strand
    gene_id = case_when(
      # If ref strand is NA, (i.e. no ref known) keep the combined_gene_id
      is.na(strand.y) ~ combined_gene_id,
      # If class_code_annotated is x or i and strand is different, keep the combined_gene_id
      class_code_annotated %in% c('x', 'i') & strand.x != strand.y ~ combined_gene_id,
      # If class_code_annotated is not x or i, strand is the same as reference, and ref_gene_annotated is known, keep the ref_gene_id_annotated
      !class_code_annotated %in% c('x', 'i') & strand.x == strand.y & ref_gene_id_annotated != "-" ~ ref_gene_id_annotated,
      # Otherwise, keep the combined_gene_id
...
# gtf_novel <- subset(gtf_novel, gene_id != "-")  # Need to find out what to do with these guys, as some of them occur in quite a lot of samples
...
gtf_novel <- subset(
  gtf_novel, !(gtf_novel$transcript_id %in% ref_overlap_txs)
)

## Remove genes with exons on both strands
# 1. Subset the GTF data to retain only exon entries
exon_data <- gtf_novel[gtf_novel$type == "exon",]
...
gtf_novel <- subset(
  gtf_novel, !(gtf_novel$gene_id %in% genes_both_strands)
)
# 6. Print message with number of genes with exons on both strands
message(paste(Sys.time(), "Number of genes with exons on both strands:",
              length(genes_both_strands)), sep = "\t")
